...
import { X, Copy, Check, Eye, EyeOff, Trash2, ExternalLink, ShieldCheck, Edit, Star, CheckSquare } from 'lucide-react';
...
export default function VaultItemDetail({ account, onClose, onDeleted, onUpdated, onEditRequest }: VaultItemDetailProps) {
  ...
  const handleDelete = async () => {
    const confirmed = window.confirm(`Are you sure you want to delete ${account.account_name}? This cannot be undone.`);
    if (!confirmed) return;
    setIsDeleting(true);
    try {
        ...
        <div className="flex flex-col px-6 pt-6 pb-4 border-b border-border bg-background">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center space-x-1">
              ...
              <button onClick={onClose} className="p-2 text-text-muted hover:text-text-main rounded-md hover:bg-surface transition-colors" title="Close">
                <X className="w-5 h-5" />
              </button>
            </div>
          </div>
          ...
          <p className="text-sm text-text-muted mt-1 flex items-center">
            {account.account_type} {account.is_favorite && <span className="ml-2 px-2 py-0.5 bg-warning/10 text-warning text-xs rounded-full font-medium tracking-wide uppercase">Favorite</span>}
          </p>
        </div>
          ...
          <div className="space-y-4">
            {account.username && (
              <div className="group p-3 bg-background border border-border rounded-lg relative hover:border-primary/50 transition-colors">
                <label className="block text-xs font-medium text-text-muted mb-1 uppercase tracking-wider">Username</label>
            ...
            {account.url && (
              <div className="group p-3 bg-background border border-border rounded-lg relative hover:border-primary/50 transition-colors">
                <label className="block text-xs font-medium text-text-muted mb-1 uppercase tracking-wider">Website</label>
                <a href={account.url} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-primary hover:underline flex items-center">
                  {account.url}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center text-sm font-medium text-text-main">
                <ShieldCheck className={`w-5 h-5 mr-2 ${account.has_2fa ? 'text-success' : 'text-text-muted'}`} />
                Two-Factor Authentication
              </div>
              <span className={`text-xs font-bold uppercase tracking-wider px-2 py-1 rounded ${account.has_2fa ? 'bg-success/10 text-success' : 'bg-surface text-text-muted'}`}>
            ...
            {account.has_2fa && account.recovery_codes && account.recovery_codes.length > 0 && (
              <div className="p-4 bg-background border border-border rounded-lg">
                <label className="block text-xs font-medium text-text-muted mb-3 uppercase tracking-wider">Recovery Codes</label>
                  ...
                  {account.recovery_codes.map((rc, idx) => {
                    ...
                    return (
                      <div key={idx} className="flex items-center space-x-1">
                        ...
                        >
                          {codeString}
                          {copiedField === `code-${idx}` && <Check className="w-3 h-3 text-success" />}
                        </button>
            ...
            <>
              ...
              <div>
                <label className="block text-xs font-medium text-text-muted mb-2 uppercase tracking-wider flex justify-between items-center">
                  Secure Notes
                  <button onClick={() => copyToClipboard(notesString, 'notes')} className="text-text-muted hover:text-primary transition-colors" title="Copy Notes">
                    {copiedField === 'notes' ? <Check className="w-3 h-3 text-success" /> : <Copy className="w-3 h-3" />}
                  </button>
                </label>
                <div className="p-4 bg-background border border-border rounded-lg">
          ...
          {account.tags && account.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 pt-2">
              {account.tags.map(tag => (
                <span key={tag} className="px-2.5 py-1 bg-surface border border-border text-text-muted text-xs font-medium rounded-md">
                  #{tag}
                </span>
              ))}
            </div>
